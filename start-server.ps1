...
if ((Test-Path -LiteralPath $overlaySource -PathType Container) -and
    ...
    # Auto Lowy need one small server-side query for a target and one for
    # owned loot.  The engine tree is deliberately not stored in this repo,
    # therefore keep the implementation as a tracked fragment and install it
    # idempotently into the operator's staged source before every build.
    $engineGameSource = Join-Path $PSScriptRoot 'linux-port\docker\game\src\server\game\src'
    $autoHuntFragment = Join-Path $PSScriptRoot 'linux-port\overlays\playerbot\serverfiles\player_autohunt_commands.inc'
    $cmdGeneral = Join-Path $engineGameSource 'cmd_general.cpp'
    $cmdTable = Join-Path $engineGameSource 'cmd.cpp'
    if ((Test-Path -LiteralPath $autoHuntFragment -PathType Leaf) -and
        (Test-Path -LiteralPath $cmdGeneral -PathType Leaf) -and
        (Test-Path -LiteralPath $cmdTable -PathType Leaf)) {
        $generalText = Get-Content -LiteralPath $cmdGeneral -Raw
        if ($generalText -notmatch 'ACMD\(do_autohunt_target\)' -or
            $generalText -notmatch 'MT2009_PLUS_AUTOHUNT_V2') {
            $fragmentText = Get-Content -LiteralPath $autoHuntFragment -Raw
            $engineFooter = '//martysama0134'
            $footerAt = $generalText.IndexOf($engineFooter)
            $oldAutoHuntAt = $generalText.IndexOf('// The player''s auto-hunt')
            if ($oldAutoHuntAt -ge 0 -and $footerAt -gt $oldAutoHuntAt) {
                $generalText = $generalText.Remove(
                    $oldAutoHuntAt, $footerAt - $oldAutoHuntAt).
                    Insert($oldAutoHuntAt, $fragmentText.TrimEnd() + "`r`n`r`n")
            }
            elseif ($footerAt -ge 0) {
                $generalText = $generalText.Insert($footerAt, $fragmentText.TrimEnd() + "`r`n`r`n")
            }
            else {
                $generalText = $generalText.TrimEnd() + "`r`n`r`n" + $fragmentText
            }
            [IO.File]::WriteAllText($cmdGeneral, $generalText)
            $syncedFiles++
        }

        $cmdText = Get-Content -LiteralPath $cmdTable -Raw
        if ($cmdText -notmatch 'ACMD\(do_autohunt_target\)') {
            if ($cmdText -notmatch 'ACMD\(do_check_mob\);') {
                throw 'Nie mozna zarejestrowac Auto Lowow: cmd.cpp nie ma deklaracji do_check_mob.'
            }
            $cmdText = $cmdText.Replace(
                'ACMD(do_check_mob);',
                "ACMD(do_check_mob);`r`nACMD(do_autohunt_target);`r`nACMD(do_autohunt_loot);")
        }
        if ($cmdText -notmatch '"autohunt_target"') {
            $checkMobLine = [regex]::Match($cmdText, '(?m)^[ \t]*\{[ \t]*"check_mob"[^\r\n]*\r?\n')
            if (-not $checkMobLine.Success) {
                throw 'Nie mozna zarejestrowac Auto Lowow: cmd.cpp nie ma wpisu check_mob.'
            }
            $entries = $checkMobLine.Value +
                "`t{ `"autohunt_target`",`tdo_autohunt_target,`t0,`t`t`tPOS_DEAD,`tGM_PLAYER`t},`r`n" +
                "`t{ `"autohunt_loot`",`tdo_autohunt_loot,`t0,`t`t`tPOS_DEAD,`tGM_PLAYER`t},`r`n"
            $cmdText = $cmdText.Remove($checkMobLine.Index, $checkMobLine.Length).
                Insert($checkMobLine.Index, $entries)
        }
        $oldCmdText = Get-Content -LiteralPath $cmdTable -Raw
        if ($cmdText -ne $oldCmdText) {
            [IO.File]::WriteAllText($cmdTable, $cmdText)
            $syncedFiles++
        }
    }
