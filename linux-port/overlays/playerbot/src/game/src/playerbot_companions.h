...
namespace
{
	...
	bool IsPlayerBotMercAvailable(LPCHARACTER ch, const TPlayerBotAIState& state, DWORD dwNow)
	{
		...
		if (state.bVisitingShop || state.bFishingSession || state.bMarketTrip ||
				...
				state.bVisitingAlchemist || state.bVisitingUriel || state.bSaddlebagErrand != 0 || state.bVisitingDsAlchemist || state.bServicePending || state.bRecoveringAfterDeath || state.bTacticalRetreat ||
